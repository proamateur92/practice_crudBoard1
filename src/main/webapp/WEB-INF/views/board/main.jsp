			...
			<table>
				<tr><th>번호</th><th>제목</th><th>작성자</th><th>등록일</th></tr>
					<c:forEach var="boardInfo" items="${boardInfo }" >
						<tr>
							<td><c:out value="${boardInfo.bno }"/></td>
							<td>
								<a href="/board/readView?bno=${boardInfo.bno}&
														page=${scri.page}&
														perPageNum=${scri.perPageNum}&
														searchType=${scri.searchType}&
														keyword=${scri.keyword}"><c:out value="${boardInfo.title}" /></a>
							</td>
			...
			<div>
			  <ul>
			    ...
			    <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
			    	<li><a href="main${pageMaker.makeSearch(idx)}">${idx}</a></li>
			    </c:forEach>
